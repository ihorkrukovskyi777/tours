...
export default async function BannerCity({id, locale, size, isMobile = false, pageTitle = null}) {
    ...
    return (
        ...
        >
            <p className="pick_text pick_text--md">{i18n.t('See the best Free Tours of {city} offered by local tour guides')?.replace('{city}', pageTitle).replaceAll('<br>', '').replaceAll('</br>', '')}</p>
